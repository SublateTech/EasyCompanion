...
public class ShareWithActivity extends XmppActivity implements XmppConnectionService.OnConversationUpdate {
	...
	private eu.siacs.conversations.ui.UiCallback<Message> attachFileCallback = new UiCallback<Message>() {
				...
				public void run() {
					if (attachmentCounter.decrementAndGet() <=0 ) {
						...
						if (share.image && share.multiple) {
							resId = eu.siacs.conversations.R.string.shared_images_with_x;
						} else if (share.image) {
							resId = eu.siacs.conversations.R.string.shared_image_with_x;
						} else {
							resId = eu.siacs.conversations.R.string.shared_file_with_x;
						}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(eu.siacs.conversations.app.R.layout.share_with);
		setTitle(getString(eu.siacs.conversations.R.string.title_activity_sharewith));

		mListView = (ListView) findViewById(eu.siacs.conversations.app.R.id.choose_conversation_list);
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		getMenuInflater().inflate(eu.siacs.conversations.app.R.menu.share_with, menu);
		return true;
	}
	...
	public boolean onOptionsItemSelected(final MenuItem item) {
		switch (item.getItemId()) {
			case eu.siacs.conversations.app.R.id.action_add:
	...
	private void share(final Conversation conversation) {
		...
		if (conversation.getNextEncryption() == Message.ENCRYPTION_PGP && !hasPgp()) {
			if (share.uuid == null) {
				...
				Toast.makeText(this, eu.siacs.conversations.R.string.openkeychain_not_installed, Toast.LENGTH_SHORT).show();
				finish();
			}
			return;
		}
			...
			OnPresenceSelected callback = new OnPresenceSelected() {
				...
				public void onPresenceSelected() {
					...
					if (share.image) {
						...
						replaceToast(getString(share.multiple ? eu.siacs.conversations.R.string.preparing_images : eu.siacs.conversations.R.string.preparing_image));
						...
						replaceToast(getString(eu.siacs.conversations.R.string.preparing_file));
						ShareWithActivity.this.xmppConnectionService
			...
			if (mReturnToPrevious && this.share.text != null && !this.share.text.isEmpty() ) {
				final OnPresenceSelected callback = new OnPresenceSelected() {
					...
					public void onPresenceSelected() {
						...
						xmppConnectionService.sendMessage(message);
						replaceToast(getString(eu.siacs.conversations.R.string.shared_text_with_x, conversation.getName()));
						finish();
					}
				};
				if (conversation.getNextEncryption() == Message.ENCRYPTION_OTR) {
	...
	public void onBackPressed() {
		if (attachmentCounter.get() >= 1) {
			replaceToast(getString(eu.siacs.conversations.R.string.sharing_files_please_wait));
		} else {
			super.onBackPressed();
		}
	}
