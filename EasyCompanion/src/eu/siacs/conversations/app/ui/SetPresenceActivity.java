...
public class SetPresenceActivity extends XmppActivity implements View.OnClickListener {
	...
	protected void onCreate(final Bundle savedInstanceState) {
		...
		setContentView(eu.siacs.conversations.app.R.layout.activity_set_presence);
		mScrollView = (ScrollView) findViewById(eu.siacs.conversations.app.R.id.scroll_view);
		mShowSpinner = (Spinner) findViewById(eu.siacs.conversations.app.R.id.presence_show);
		ArrayAdapter adapter = ArrayAdapter.createFromResource(this,
				eu.siacs.conversations.app.R.array.presence_show_options,
				eu.siacs.conversations.app.R.layout.simple_list_item);
		mShowSpinner.setAdapter(adapter);
		mShowSpinner.setSelection(1);
		mStatusMessage = (EditText) findViewById(eu.siacs.conversations.app.R.id.presence_status_message);
		mAllAccounts = (CheckBox) findViewById(eu.siacs.conversations.app.R.id.all_accounts);
		mTemplatesView = (LinearLayout) findViewById(eu.siacs.conversations.app.R.id.templates);
		final Button changePresence = (Button) findViewById(eu.siacs.conversations.app.R.id.change_presence);
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		getMenuInflater().inflate(eu.siacs.conversations.app.R.menu.change_presence, menu);
		return super.onCreateOptionsMenu(menu);
	}

	@Override
	public boolean onOptionsItemSelected(final MenuItem item) {
		if (item.getItemId() == eu.siacs.conversations.app.R.id.action_account_details) {
			if (mAccount != null) {
	...
	private void redrawTemplates() {
		if (mTemplates == null || mTemplates.size() == 0) {
			...
			for (PresenceTemplate template : mTemplates) {
				View templateLayout = inflater.inflate(eu.siacs.conversations.app.R.layout.presence_template, mTemplatesView, false);
				templateLayout.setTag(template);
				setListItemBackgroundOnView(templateLayout);
				templateLayout.setOnClickListener(this);
				TextView message = (TextView) templateLayout.findViewById(eu.siacs.conversations.app.R.id.presence_status_message);
				TextView status = (TextView) templateLayout.findViewById(eu.siacs.conversations.app.R.id.status);
				ImageButton button = (ImageButton) templateLayout.findViewById(eu.siacs.conversations.app.R.id.delete_button);
	...
	public void onClick(View v) {
		...
		if (v.getId() == eu.siacs.conversations.app.R.id.presence_template) {
		...
		} else if (v.getId() == eu.siacs.conversations.app.R.id.delete_button) {
