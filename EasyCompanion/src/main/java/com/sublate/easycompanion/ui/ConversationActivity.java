...
import com.sublate.easycompanion.utils.ExceptionHelper;

import static com.sublate.easycompanion.Config.LOGTAG;
...
public class ConversationActivity extends XmppActivity
	...
	protected void onCreate(final Bundle savedInstanceState) {
		...
		listView.setDismissCallback(new EnhancedListView.OnDismissCallback() {
			...
			public EnhancedListView.Undoable onDismiss(final EnhancedListView enhancedListView, final int position) {
				...
				final int index = listView.getFirstVisiblePosition();View v = listView.getChildAt(0);
	...
	public boolean onContextItemSelected(MenuItem item) {
		...
		switch (item.getItemId()) {
			...
			case R.id.user_profile_location:
				String fallbackPackageId = "com.sublate.easycompanion.friendslocation";
				...
				if (isInstalled) {
					...
					if (contact.getPresences().size() > 0) {
						String resource = contact.getPresences().toResourceArray()[0];
						try {
							Jid jid = Jid.fromParts(conversation.getContact().getJid().getLocalpart(), conversation.getContact().getJid().getDomainpart(), resource);
							contact = new Contact(jid);

							Intent intent = new Intent(Intent.ACTION_VIEW);
							intent.setAction("com.sublate.easycompanion.friendslocation.request");
							intent.putExtra("fromJid", conversation.getAccount().getJid().toFullinJid().toString());
						...
						Toast.makeText(ConversationActivity.this,getString(R.string.missing_presence_online), Toast.LENGTH_SHORT).show();
					}
				...
				else
				{
					...
					Intent intent = getInstallApkIntent(fallbackPackageId);
					startActivity(intent);
				}
				break;
            case R.id.user_get_routes:
                final Conversation conversation = getSelectedConversation();
                final Contact contact = conversation.getContact();

				//selectPresenceToAttachFile(ATTACHMENT_CHOICE_CHOOSE_FILE, Message.ENCRYPTION_NONE);
                ...
                if (contact.getPresences().size() > 0) {
                    ...
                    try {
                        ...
                        Intent intent = new Intent(XmppConnectionService.ACTION_GEOLOC_GET_ROUTE);
                        intent.putExtra("fromJid", conversation.getAccount().getJid().toFullinJid().toString());
                        intent.putExtra("toJid", contact.getJid().toFullinJid().toString());
                        sendBroadcast(intent);
                        Log.d(LOGTAG, "Sending :" + intent.getAction());
			...
			case R.id.user_get_order:
				final Conversation conversation_1 = getSelectedConversation();
				final Contact contact_1 = conversation_1.getContact();

				//selectPresenceToAttachFile(ATTACHMENT_CHOICE_CHOOSE_FILE, Message.ENCRYPTION_NONE);
				selectPresence(conversation_1, new OnPresenceSelected() {
					@Override
					public void onPresenceSelected() {
						if (contact_1.getPresences().size() > 0) {
							String resource = contact_1.getPresences().toResourceArray()[0];
							try {
								Jid jid = Jid.fromParts(conversation_1.getContact().getJid().getLocalpart(), conversation_1.getContact().getJid().getDomainpart(), resource);
								Contact contact = new Contact(jid);

								Intent intent = new Intent(XmppConnectionService.ACTION_COMMAND_CREATE_ORDER);
								intent.putExtra("fromJid", conversation_1.getAccount().getJid().toFullinJid().toString());
								intent.putExtra("toJid", contact.getJid().toFullinJid().toString());
								sendBroadcast(intent);
								Log.d(LOGTAG, "Sending :" + intent.getAction());
			...
			public void onPresenceSelected() {
				...
				switch (attachmentChoice) {
					...
					case ATTACHMENT_CHOICE_RECORD_VOICE:
						...
						fallbackPackageId = "com.sublate.easycompanion.voicerecorder";
						break;
					case ATTACHMENT_CHOICE_LOCATION:
						intent.setAction("com.sublate.easycompanion.location.request");
						fallbackPackageId = "com.sublate.easycompanion.friendslocation";
						break;
				}
	...
	protected void attachFileDialog() {
		...
		if (new Intent("com.sublate.easycompanion.location.request").resolveActivity(getPackageManager()) == null) {
