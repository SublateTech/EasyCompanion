 ...
 * killing the process.
 * </p><p>
 * A service's login purpose is to do some task (usually in the background) which requires one or
 ...
 * thread and task queuing for all TrackerApp services to use.
 * </p><p>
...
public abstract class CoreService extends Service {
    ...
    public static String WAKE_LOCK_ID = "com.sublate.chatgps.service.CoreService.wakeLockId";
    ...
    protected static void addWakeLock(Context context, Intent intent) {
        TracingPowerManager.TracingWakeLock wakeLock = acquireWakeLock(context, "CoreService addWakeLock",
                Config.LOCATION_SERVICE_WAKE_LOCK_TIMEOUT);
    ...
    public void onCreate() {
        if (Config.DEBUG) {
            Log.i(Config.LOGTAG, "CoreService: " + className + ".onCreate()");
        }
    ...
    public final int onStartCommand(Intent intent, int flags, int startId) {
         ...
         * services are started from TrackerConfig.onCreate() when the Application object is initialized.
         *
         * See issue 3750
         */
        if (intent == null) {
        ...
        if (Config.DEBUG) {
            Log.i(Config.LOGTAG, "CoreService: " + className + ".onStart(" + intent + ", " + startId + ")");
        }
        ...
        if (coreWakeLockId != -1) {
            if (Config.DEBUG) {
                Log.d(Config.LOGTAG, "Got core wake lock id " + coreWakeLockId);
            }
            ...
            if (coreWakeLock != null) {
                if (Config.DEBUG) {
                    Log.d(Config.LOGTAG, "Found core wake lock with id " + coreWakeLockId +
                            ", releasing");
                }
    ...
    public void execute(Context context, final Runnable runner, int wakeLockTime,
        ...
        Runnable myRunner = new Runnable() {
            public void run() {
                try {
                    // Get the sync status
                    boolean oldIsSyncDisabled = false; //MailService.isSyncDisabled();

                    if (Config.DEBUG) {
                        Log.d(Config.LOGTAG, "CoreService (" + className + ") running Runnable " +
                    ...
                    */
                    ...
                    try {
                        if (Config.DEBUG) {
                            Log.d(Config.LOGTAG, "CoreService (" + className + ") completed " +
        ...
        if (mThreadPool == null) {
            Log.e(Config.LOGTAG, "CoreService.execute (" + className + ") called with no thread " +
            ...
            if (Config.DEBUG) {
                Log.d(Config.LOGTAG, "CoreService (" + className + ") queueing Runnable " +
                        runner.hashCode() + " with startId " + startId);
            }
            ...
            try {
                ...
                Log.i(Config.LOGTAG, "CoreService: " + className + " is shutting down, ignoring " +
    ...
    public void onLowMemory() {
        Log.w(Config.LOGTAG, "CoreService: " + className + ".onLowMemory() - Running low on memory");
    }
    ...
    public void onDestroy() {
        if (Config.DEBUG) {
            Log.i(Config.LOGTAG, "CoreService: " + className + ".onDestroy()");
        }
