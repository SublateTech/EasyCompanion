...
public class PresenceGenerator extends eu.siacs.conversations.generator.AbstractGenerator {
	...
	private PresencePacket subscription(String type, Contact contact, Jid jid) {
		PresencePacket packet = new PresencePacket();
		packet.setAttribute("type", type);
		packet.setTo(contact.getJid().toFullinJid());
		packet.setFrom(jid.toFullinJid());
	...
	public PresencePacket requestPresenceUpdatesFrom(Contact contact, Jid jid) {
		return subscription("subscribe", contact, jid);
	}
