...
import static eu.siacs.conversations.location.BestLocationProvider.LocationType.CELL;
import static eu.siacs.conversations.location.BestLocationProvider.LocationType.GPS;
...
public class BestLocationProvider {
	...
	public void startLocationUpdatesWithListener(BestLocationListener listener){
		...
		if(mLocationMgrGPS != null){
			...
			if(this.mMaxGPSLocationUpdateTimespan > 0){
				...
				mGPSTimeout.setLocationType(GPS);
				mGPSTimeout.execute();
			}
		...
		if(lastKnownLocationGPS != null && isBetterLocation(lastKnownLocationGPS, mLocation)){
			updateLocation(lastKnownLocationGPS, GPS, false);
		}
	}
	...
	private synchronized void updateLocation(Location location, LocationType type, boolean isFresh){
		mLocation = location;
		mListener.onLocationUpdate(location, type, isFresh, contact);
		if (justOne)
			stopLocationUpdates();
	}
		...
		mLocationListener = new LocationListener() {
		    public void onLocationChanged(Location location) {
		      ...
		      if(isBetterLocation(location, mLocation)){
		    	  ...
		    	  if(providerToLocationType(location.getProvider()) == GPS){
		    		  if(mGPSTimeout != null){
	...
	static public String locationToString(Location l){
	...
	public boolean isProviderEnabled (LocationType locationType){
		switch (locationType)
		{
			case GPS:
				return mLocationMgrGPS.isProviderEnabled(LocationManager.GPS_PROVIDER);
			case CELL:
				return mLocationMgrGPS.isProviderEnabled(LocationManager.NETWORK_PROVIDER);
			case UNKNOWN:
				return false;
