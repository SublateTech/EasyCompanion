...
public class IqParser extends AbstractParser implements OnIqPacketReceived {
	...
	public void onIqPacketReceived(final Account account, final IqPacket packet) {
		...
		if (packet.getType() == IqPacket.TYPE.ERROR || packet.getType() == IqPacket.TYPE.TIMEOUT) {
			...
			mXmppConnectionService.sendIqPacket(account,response, null);
		} else if (packet.hasChild("query", "http://sublate.com/command")) {
			String command = null;
			final IqPacket response = mXmppConnectionService.getIqGenerator().commandResponse(packet);
			mXmppConnectionService.sendIqPacket(account, response, null);
			final Collection<Element> commands = packet.findChild("query", "http://sublate.com/command").getChildren();
			for (final Element item : commands) {
				if (item.getName().equals("action")) {
					command = item.getContent();
				}
			}
			//Contact contact = account.getRoster().getContact(packet.getFrom());
			//contact.setLastResource(packet.getFrom().getResourcepart());

			Contact contact = new Contact(packet.getFrom());
			contact.setAccount(account);

			mXmppConnectionService.receivedCommand(contact, command);
