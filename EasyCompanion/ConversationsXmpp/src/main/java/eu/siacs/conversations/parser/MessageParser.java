...
public class MessageParser extends AbstractParser implements OnMessagePacketReceived {
	...
	public void onMessagePacketReceived(Account account, MessagePacket original) {
		...
		final Element geoMessage = packet.findChild("geoloc", "http://jabber.org/protocol/geoloc");
		if (geoMessage!=null)
		{
			Contact contact = account.getRoster().getContact(from);
			//contact.setPresenceName(nick);
			Geoloc geoloc = new Geoloc();
			geoloc.setContact(contact);

			for (Element child : geoMessage.getChildren()) {
				if ("lon".equals(child.getName())) {
					double lon = Double.parseDouble(child.getContent());
					geoloc.setLon(lon);
				} else if ("lat".equals(child.getName())) {
					double lat = Double.parseDouble(child.getContent());
					geoloc.setLat(lat);
				} else if ("alt".equals(child.getName())) {
					double alt = Double.parseDouble(child.getContent());
					geoloc.setAlt(alt);
				}
			}
			mXmppConnectionService.processGeolocMessage(geoloc);
		}
	}
